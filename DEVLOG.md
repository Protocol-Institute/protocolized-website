...
## Session 21: Series Nav Fix (Legends & Ledgers Ch. 3) + Reusable Series-TOC Script

*2026-07-18*

**Tracks:** content-sync, operations

- A working-tree change to `worker/src/html/feature-new-nature.tsx` removing the "Money Matters" section (funding/budget paragraph, slide-27 and slide-28 figures, support-page link) had been sitting uncommitted since Session 20. Reviewed the diff, confirmed it was an intentional content edit rather than accidental loss, and committed it. No functional change to the Worker beyond the essay body content.

- Elizabeth Maher's third Legends &amp; Ledgers story, "The Big Man," published on Substack 2026-06-12 and was already mirrored into the `posts` table and live at `/p/the-big-man` — but it never got series navigation. Root cause: the original series wiring (Session 14) tagged posts with `series_slug`/`series_position` via one-off `wrangler d1 execute` commands run directly against production, with no script or migration file to re-run for later chapters. Any story published after that initial pass would silently lack series nav unless someone remembered to hand-write another UPDATE. Confirmed via direct D1 query that only `all-you-can-do-here-is-leave` (position 1) and `troll` (position 2) carried the series tag; `the-big-man` had both fields `NULL`. Fixed by setting `series_slug='legends-and-ledgers'`, `series_position=3` on the post, and appending the corresponding entry to `books.legends-and-ledgers.toc` (the JSON array `getSeriesContext` reads for both `totalParts` and TOC-fallback prev/next links — see `worker/src/db.ts`). Verified live: the post now renders "Part 3 of 3" with working prev/next links in both directions.

- Since the series-wiring step has now bitten twice (undocumented at seed time, then silently skipped for a later chapter), wrote a script that makes it a repeatable, reviewable operation instead of hand-written SQL. Given `--book` and `--post` slugs, it reads the current `books.toc` and the post's row via `wrangler d1 execute --json`, infers title/author/position when not passed explicitly (author defaults to the last existing TOC entry's author; position defaults to `len(toc)+1`), and applies both the TOC append and the post's series fields in a single SQL file via wrangler. Refuses to run if the post already carries a `series_slug`, so it can't double-add a chapter. This is the pattern to reach for whenever a new chapter goes into any of the 4 Jamverse series (Trainverse, Legends &amp; Ledgers, Zoothesia, Stockton Chronicles) going forward — no more direct production UPDATEs for this.

---

## Session 22: Fiction/Nonfiction Publication Split — Segregation Phases 0-3

*2026-08-17*

**Tracks:** framework, operations

- Bumped `hono` 4.7.0&rarr;4.12.33 in `worker/`, resolving 8 npm audit advisories flagged after an exe.dev VM install. Lockfile-only change, no code touched &mdash; verified via `tsc --noEmit` and `wrangler deploy --dry-run` in the original PR description. The remaining 7 advisories are dev-toolchain-only (wrangler/tailwind, never in the deployed bundle) and were deliberately left for a separate PR since fixing them requires a wrangler major bump with a breaking peer-dependency change.

- Protocolized (this site + its original Substack) is splitting into a **nonfiction publication** that keeps this domain, name, and the original Substack, and a **new fiction publication** (working name "Monstrous Times," own domain TBD &mdash; deliberately not hardcoded anywhere in code). The existing Jamverse project (jamverse.protocolized.io, separate repo) is not itself the new pub but will become a subdomain of it. Decision: **segregate now, decommission later** &mdash; fiction content stays live at its current URLs until Monstrous Times has a real destination; this session's work makes the codebase ready to flip a switch later, without changing anything visible today.

- Three parallel Explore passes over the Worker, the Astro fallback, and the sync scripts found that "is this fiction" was an emergent property of three uncoordinated fields &mdash; `posts.section = 'Fictions'` (61 posts), `resources.type = 'fiction'` (65 resources), and `books.category = 'fiction'` (8 of 10 books) &mdash; plus `posts.series_slug` joined to fiction books via `getSeriesContext`. Every list-producing route in the live Worker (`/`, `/magazine`, `/books`, `/resources`, `/sitemap.xml`, `/rss.xml`, `/llms.txt`, `/api/resources.json`, the homepage carousel) was unfiltered by any of these fields. The two automated ingestion paths (`sync-substack.yml` daily cron, `sync-resources-d1.yml` on push) were also completely type-agnostic &mdash; they would keep pulling fiction content in indefinitely if any were ever posted to the original Substack again, with zero code change required. The Astro fallback pages are confirmed dead in CI (nothing builds/deploys them), but `src/content/resources/*.md` is not dead &mdash; `migrate-to-d1.py` globs and ingests every file there unconditionally into the live D1 the Worker reads from.

- Reclassified 3 Jamverse meta/announcement posts (`jamverse-jam`, `jamverse-live`, `jamverse-jam-contest-closes-today`) from `section='Protocolized'` to `'Fictions'`, per Venkat's call that Jamverse meta content moves with fiction while the `Obliquities` section (Rao's speculative nonfiction essays) stays. While auditing anthology-tagged resources, found `ghosts-in-machines` and its epub variant were mistyped `type='article'`/`'paper'` despite being the fiction anthology itself (confirmed via cross-reference against the `books` table, where `ghosts-in-machines` is `category='fiction'`) &mdash; corrected to `type='fiction'` in both D1 and the source Markdown frontmatter (5 files total), so a future `migrate-to-d1.py` run won't revert it. A broader audit of ~45 nonfiction items tagged `fiction` (talks, editorials, Rao's Obliquities essays discussing fiction as a topic) confirmed the `fiction` *tag* is used loosely as a topic marker and is **not** a reliable classification signal &mdash; only `type`/`section`/`category` are authoritative. New single-source-of-truth module: `worker/src/fiction.ts` (TypeScript) and `scripts/fiction_classification.py` (Python, mirrors the same constants) so the Worker and the sync scripts agree on the boundary.

- New D1 table `fiction_redirects` (slug &rarr; destination URL, kind, currently empty) is the redirect source of truth once fiction moves &mdash; deliberately a per-slug table rather than a single base-URL substitution, since Jamverse-branded content and standalone fiction pieces will likely route to different destinations once Jamverse becomes a subdomain of Monstrous Times. Critically, it's decoupled from the content tables (`posts`/`resources`/`books`) so redirects keep working even after those rows are eventually deleted in Phase 4. New env var `FICTION_SEGREGATION_ACTIVE` in `worker/wrangler.toml` (currently `"false"`) gates every fiction-touching query in `worker/src/db.ts` (`getAllResources`, `getAllBooks`, `getLatestPosts`, `getRandomArchiveResources`, `getRandomBooks`, `getRelatedResources`, `getAdjacentPosts`, `getAnthologies`) and every route in `worker/src/index.tsx` that touches them, plus direct-access routes `/p/:slug`, `/resources/:slug`, `/books/:slug` which check `fiction_redirects` before falling through to normal lookup. Verified locally via `wrangler dev --remote` against live production D1 in both flag states: flag `true` &rarr; sitemap drops from 471 to 331 URLs with zero fiction slugs present, `/anthologies` 301s (it's entirely fiction-branded copy, so there's nothing left to show once fiction is excluded), a manually-inserted test `fiction_redirects` row correctly 301s `/p/jamverse-jam`; flag `false` &rarr; output is byte-for-byte consistent with today's production behavior (471 URLs, 70 fiction resources present, fiction framing intact in `/llms.txt` copy). Zero visible change shipped to production this session.

- `sync-substack.py` and `sync-substack-resources.py` now print a loud (non-blocking) warning if they ever encounter a `Fictions`-section post, since post-fork this should never happen on the original Substack. Deliberately a warning, not a hard fail &mdash; a taxonomy edge case shouldn't silently drop legitimate content from an automated cron run. `sync-substack-resources.py`'s warning is deduplicated per section per run (not per slug) since the existing 61+ fiction posts get re-processed on every manual run of that script by design (no skip/filter logic exists), so a naive per-slug warning would have been unusably noisy.

- New `scripts/export-fiction-bundle.py` queries D1 for all fiction posts/resources/books, downloads every referenced R2 asset (post body HTML, cover images, resource files/thumbnails, book covers/banners/epubs) over plain HTTP from `files.protocolized.io` (simpler than invoking `wrangler r2 object get` per-file), and writes a self-contained `manifest.json` + assets directory, zipped. Ran once now as a baseline: **64 posts, 70 resources, 8 books, 129 R2 assets** &rarr; `exports/fiction-bundle-2026-08-17.zip` (193MB, gitignored &mdash; a handoff artifact, not a repo asset). Documented as "re-run immediately before the actual cutover" so the handoff bundle stays fresh.
...
- **(1) The fiction export bundle is stale.** `exports/fiction-bundle-2026-08-17.zip` predates the section repair and under-counts fiction by six posts — handing it to Monstrous Times as-is would ship an incomplete archive. Phase 4 now names re-running `export-fiction-bundle.py` as required, not optional. **(2) Series wiring fails silently and has now done so twice** (&quot;The Big Man&quot; caught in Session 21, &quot;The Dismantling&quot; in Session 23). The post mirrors and renders normally; only the series nav and book TOC are missing, so nothing surfaces it. Logged a detection query (fiction posts with no series whose author already has one — deliberately noisy, a review prompt rather than an alarm) and an infra item proposing that `sync-substack.py` print a notice at sync time, when the decision is cheap, instead of relying on someone noticing weeks later. Left as a notice rather than a hard fail because the same authors legitimately write standalone fiction.
