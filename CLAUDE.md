...
- **New chapters in an existing series are not wired automatically.** The Substack sync does not infer series membership; run `scripts/update-series-toc.py` by hand after a new chapter publishes, or it silently stays out of the series nav and book TOC.
